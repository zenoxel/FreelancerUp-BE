...
```bash
curl -X POST http://localhost:8080/api/v1/projects/project123/bids \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "http://localhost:8080/api/v1/projects/search?keyword=mobile&page=0&size=10&sort=createdAt,desc"
```
